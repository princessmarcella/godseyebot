...
import godseye.modules.sql.notes_sql as sql
from godseye import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from godseye.__main__ import DATA_IMPORT
from godseye.modules.helper_funcs.chat_status import user_admin
from godseye.modules.helper_funcs.misc import build_keyboard, revert_buttons
from godseye.modules.helper_funcs.msg_types import get_note_type
from godseye.modules.rules import get_rules
import godseye.modules.sql.rules_sql as rulessql
from godseye.modules.sql import warns_sql as warnssql
import godseye.modules.sql.blacklist_sql as blacklistsql
from godseye.modules.sql import disable_sql as disabledsql
from godseye.modules.sql import cust_filters_sql as filtersql
import godseye.modules.sql.welcome_sql as welcsql
import godseye.modules.sql.locks_sql as locksql
from godseye.modules.connection import connected
...
def export_data(bot: Bot, update: Update, chat_data):
    ...
    f=open("godseye{}.backup".format(chat_id), "w")
    ...
    try:
        ...
        pass
    bot.sendDocument(current_chat_id, document=open('godseye{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This  is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
    os.remove("Sera{}.backup".format(chat_id)) # Cleaning file
