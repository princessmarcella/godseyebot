...
from godseye import dispatcher, OWNER_ID, SUDO_USERS, DEV_USERS, WHITELIST_USERS, LOGGER, MESSAGE_DUMP
from godseye.modules.helper_funcs.handlers import CMD_STARTERS
from godseye.modules.helper_funcs.misc import is_module_loaded, send_to_list
from godseye.modules.helper_funcs.chat_status import is_user_admin
from godseye.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from godseye.modules.helper_funcs.string_handling import markdown_parser
from godseye.modules.disable import DisableAbleCommandHandler

import godseye.modules.sql.feds_sql as sql

from godseye.modules.connection import connected
from godseye.modules.helper_funcs.alternate import send_message
# fetching...
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "godseye_fbanned_users.json"
                update.effective_message.reply_document(document=output, filename="godseye_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "godseye_fbanned_users.csv"
                update.effective_message.reply_document(document=output, filename="godseye_fbanned_users.csv",
