...
from godseye import dispatcher, LYDIA_API, OWNER_ID
import godseye.modules.sql.lydia_sql as sql
...
def add_chat(bot: Bot, update: Update):
    ...
    if not is_chat:
        ...
        msg.reply_text("Successfully enabled AI for this chat!")
    else:
...
def remove_chat(bot: Bot, update: Update):
    ...
    if not is_chat:
        ...
        msg.reply_text("Disabled AI successfully!")
        
        
def check_message(bot: Bot, message):
    reply_msg = message.reply_to_message
    if message.text.lower() == "godseye":
        return True
    if reply_msg:
