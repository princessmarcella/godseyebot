...
from godseye import dispatcher, WHITELIST_USERS, SUPPORT_USERS, SUDO_USERS, DEV_USERS, OWNER_ID, TOKEN
from godseye.__main__ import IMPORTED, HELPABLE, MIGRATEABLE, STATS, USER_INFO, DATA_IMPORT, DATA_EXPORT, CHAT_SETTINGS, USER_SETTINGS 
from godseye.modules.disable import DisableAbleCommandHandler
from godseye.modules.log_channel import loggable, gloggable
from godseye.modules.helper_funcs.chat_status import bot_admin, can_promote, user_admin, can_pin, sudo_plus, dev_plus, connection_status
from godseye.modules.helper_funcs.extraction import extract_user, extract_user_and_text
...
def load(bot: Bot, update: Update):
    ...
    try:
        imported_module = importlib.import_module("godseye.modules." + text)
    except:
...
def listmodules(bot: Bot, update: Update):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("godseye.modules.", 1)[1]
